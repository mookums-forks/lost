...
./zig-out/bin/lost pipeline --generate 1 || exit 1
# not sure why the parens are needed here, but they are
./zig-out/bin/lost pipeline --generate 1 --pixel-size 5 --focal-length 100 --fov 17 2>/dev/null && exit 1
# also throw an error if we get pixel size or focal length without the other:
./zig-out/bin/lost pipeline --generate 1 --pixel-size 5 2>/dev/null && exit 1
./zig-out/bin/lost pipeline --generate 1 --focal-length 100 2>/dev/null && exit 1
...
script -c './zig-out/bin/lost pipeline --generate 1 --centroid-algo cog --compare-centroids -' /dev/null | grep centroids_mean_error || exit 1
script -c './zig-out/bin/lost pipeline --generate 1 --centroid-algo cog --compare-centroids stdout' /dev/null | grep centroids_mean_error || exit 1
# `script` captures stderr by default
script -c './zig-out/bin/lost pipeline --generate 1 --plot-raw-input -' /dev/null | grep 'WARNING' || exit 1

echo 'Issue #32: Unenlightening error message when neither --generate nor --png is passed to pipeline'
./zig-out/bin/lost pipeline 2>&1 | grep ERROR || exit 1

echo 'Comparator assertions'
./zig-out/bin/lost pipeline --generate 1 --plot-output /dev/null 2>&1 | grep -Fe '--plot-output' || exit 1
./zig-out/bin/lost pipeline --generate 1 --centroid-algo cog --plot-output /dev/null 2>&1 | grep -Fe '--plot-output' && exit 1

echo 'Issue #36: Cog and Attitude without Star-ID'
./zig-out/bin/lost pipeline --generate 1 --centroid-algo cog --attitude-algo triad && exit 1

echo 'Run the generator without centroids a whole bunch and make sure no assertions go off'
./zig-out/bin/lost pipeline --generate 200 --generate-perturb-centroids 5 --generate-centroids-only

echo 'Error message for database that does not exist'
no_database_out=$(./zig-out/bin/lost pipeline --generate 1 --database 'does not.exist' --star-id-algo py 2>&1) && exit 1
...
nineteen_out=$(./zig-out/bin/lost pipeline --generate 19 --generate-centroids-only --attitude-algo quest --print-speed -)
...
twenty_out=$(./zig-out/bin/lost pipeline --generate 20 --generate-centroids-only --attitude-algo quest --print-speed -)
